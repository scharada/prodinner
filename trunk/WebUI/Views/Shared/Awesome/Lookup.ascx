...
<script type="text/javascript">
    ...
    $(function () {
        ...
        ae_popup('lp'+o, <%=Model.Width %>, <%=Model.Height %>, '<%=AwesomeTools.JsEncode(Model.Title) %>', true, 'center', true, {'<%=AwesomeTools.JsEncode(Model.ChooseText) %>': function () {ae_lookupChoose('<%=o %>', '<%=Url.Action("Get", Model.Controller) %>', '<%=sel %>');},'<%=AwesomeTools.JsEncode(Mui.Cancel) %>': function () { $(this).dialog('close'); }}, <%=Model.Fullscreen.ToString().ToLower() %>);
