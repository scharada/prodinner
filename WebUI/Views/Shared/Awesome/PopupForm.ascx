...
<script type="text/javascript">
        $(function() {ae_popup('<%=o %>',<%=Model.Width %>, <%=Model.Height %>, '<%=Model.Title %>', true, <%=Model.Position %>, true, {"<%=Model.CancelText %>": function () { $(this).dialog('close'); },"<%=Model.OkText %>": function () { $("#<%=o %> form").submit(); } });});
